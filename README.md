...
## Production configuration: local LLM and the corporate quality management system

Two clarifications now reflected in the report and the deck:

**Local LLM.** The proposal committed to running on the institution's on-premises model,
and that is the production configuration. The academic demonstrator uses an API key against
a hosted OpenAI-compatible inference router only because free public hosting provides no
GPU, and only with the de-identified inventory. In production the same OpenAI-compatible
client points at a local vLLM endpoint, or the Ollama backend is selected, and the model
runs on the bank's own GPU server — no prompt, retrieved extract or inventory record leaves
the building.

**Quality management system.** In production the Layer 2 policy corpus is not a folder on disk: it is sourced
from the corporate quality document management system that holds the authoritative,
version-controlled and approved procedures. This gives the retrieval index three
properties a compliance assistant needs — it quotes the approved version rather than an
exported copy; ingestion, re-indexing and withdrawal follow the document lifecycle so a
superseded provision cannot be cited; and each document's access rights are inherited by
the vector store so retrieval cannot surface a document a user could not open directly.
These requirements were written into the proposed retrieval-corpus standard assigned to the
Data Governance Service.


## Data framing and deployment status (corrections)

Two factual clarifications now applied consistently across the report, the deck and the
script:

**Data.** The inventory is neither a purely synthetic dataset nor a copy of the
institutional record. Direct use of the organisation's live inventory in an academic
deliverable was not possible, so what is used here is **the organisation's AI system
inventory with changes and additions**: entries modified, unit names generalised,
descriptions rewritten at use-case level, and further systems added. Realistic in
structure, while no individual record maps back to a real one.

**On-premises deployment.** The on-premises environment is **specified but not yet built**.
The dense FAISS retriever and the corporate quality-system corpus integration are designed
and described, not implemented. Everything reported was produced with the academic
configuration: a hosted model through an inference router, a static corpus folder, and the
modified inventory. The system therefore currently runs with the sparse half of the hybrid
retriever.
